...
const incomeRanges = [
    ...
    { label: "₹5 Lacs - ₹10 Lacs", value: "5_10_lacs", min: 500000, max: 1000000 },
    { label: "₹10 Lacs - ₹25 Lacs", value: "10_25_lacs", min: 1000000, max: 2500000 },
    { label: "₹25 Lacs - ₹50 Lacs", value: "25_50_lacs", min: 2500000, max: 5000000 },
    { label: "₹50 Lacs - ₹75 Lacs", value: "50_75_lacs", min: 5000000, max: 7500000 },
...
export default function BussinessLoanApply() {
            ...
            <div className="bg-[#CAD7FE] sm:p-8 p-6 relative">
                ...
                <div className="max-w-lg mx-auto sm:mt-10 mt-4 relative">
                    ...
                    <TypographyMuted className="mt-8 text-blue-900 leading-7">
                        Every business needs capital to grow and at Paisabazaar, we ensure you get the best of what you need. Whether you are launching or expanding your business, we pledge to give you best business loan offers.
                    </TypographyMuted>
                </div>
                <div className="hidden sm:block absolute">
                    <img src="/assets/bussiness-loan.svg" alt="personal-loan" className="w-full" />
                </div>
            </div>
                                    ...
                                    <div className="grid gap-4 sm:gap-6">
                                        {
                                            employmentOptions?.map((item) => (
                                                <div
                                                    key={item?.value}
                                                    className={`border rounded-md p-3 flex items-center justify-between gap-8 hover:bg-muted cursor-pointer ${formData.employmentType === item?.value ? "border-blue-700 bg-blue-50" : ""}`}
                                                ...
                                                >
                                                    ...
                                                    <div className={`w-5 h-5 rounded-full border ${formData.employmentType === item?.value ? 'bg-blue-700 border-blue-700' : 'border-black'}`} />
                                                </div>
                                            ))}
                            ...
                            >
                                ...
                                <Card className="shadow-none border-none sm:mt-16">
                                    <CardHeader>
                                        ...
                                        <CardTitle className='text-lg text-blue-800 mt-8'>Desired Loan Amount</CardTitle>
                                    </CardHeader>
                                    <CardContent>
                                        <div className="grid gap-4 sm:gap-6">
                                            {incomeOptions?.map((option, idx) => (
                                                <div
                                                    key={idx}
                                                    className={`border rounded-md p-3 flex items-center justify-between gap-8 hover:bg-muted cursor-pointer ${formData.desiredLoanAmount === option?.value ? "border-blue-700 bg-blue-50" : ""}`}
                                                    onClick={() => handleDesiredLoanAmount(option?.value)}
                                                >
                                                    <div className="grid gap-1">
                                                        <Label className='text-blue-800'>{option?.label}</Label>
                                                    </div>
                                                    <div className={`w-5 h-5 rounded-full border ${formData.desiredLoanAmount === option?.value ? 'bg-blue-700 border-blue-700' : 'border-black'}`} />
                                                </div>
                                            ))}
                                            {errors.desiredLoanAmount && <p className="text-red-500 text-sm">{errors.desiredLoanAmount}</p>}
                                        </div>
                                    </CardContent>
                                </Card>
                            </motion.div>
                            ...
                            >
                                <Card className="shadow-none border-none sm:mt-8">
                                    <CardHeader>
                                        ...
                                        <CardTitle className='text-lg text-blue-800 mt-8'>Gross Annual Turnover?</CardTitle>
                                    </CardHeader>
                                    <CardContent>
                                        <div className="grid gap-4 sm:gap-6">
                                            {incomeRanges?.map((option, idx) => (
                                                <div
                                                    key={idx}
                                                    className={`border rounded-md p-3 flex items-center justify-between gap-8 hover:bg-muted cursor-pointer ${formData.annualTurnOver === option?.value ? "border-blue-700 bg-blue-50" : ""}`}
                                                    onClick={() => handleAnnualTurnOver(option?.value)}
                                                >
                                                    <div className="grid gap-1">
                                                        <Label className='text-blue-800'>{option?.label}</Label>
                                                    </div>
                                                    <div className={`w-5 h-5 rounded-full border ${formData.annualTurnOver === option?.value ? 'bg-blue-700 border-blue-700' : 'border-black'}`} />
                                                </div>
                                            ))}
                            ...
                            >
                                <Card className="shadow-none border-none sm:mt-16">
                                    <CardHeader>
                                        ...
                                        <CardTitle className='text-lg text-blue-800 mt-8'>Current Business City</CardTitle>
                                    </CardHeader>
                                    <CardContent className='overflow-y-auto max-h-[80vh] pb-12'>
                                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                                            {cities?.map((city, idx) => (
                                                <div
                                                    key={idx}
                                                    className={`border rounded-md p-3 flex items-center justify-between gap-8 hover:bg-muted cursor-pointer ${formData.bussinessCity === city?.value ? "border-blue-700 bg-blue-50" : ""}`}
                                                    onClick={() => handleBussinessCity(city?.value)}
                                                >
                                                    <TypographySmall className="text-[10px]">{city?.label}</TypographySmall>
                                                    <div className={`w-5 h-5 rounded-full border ${formData.bussinessCity === city?.value ? 'bg-blue-700 border-blue-700' : 'border-black'}`} />
                                                </div>
                                            ))}
                                            {errors.bussinessCity && <p className="text-red-500 text-sm">{errors.bussinessCity}</p>}
                                        </div>
                                    </CardContent>
                                </Card>
                            </motion.div>
                            ...
                            >
                                <Card className="shadow-none border-none sm:mt-10">
                                    <CardHeader>
                                        ...
                                        <CardTitle className='text-lg text-blue-800 mt-8'>Years in Current {formData?.employmentType === 'professional' ? 'Profession' : 'Business'}</CardTitle>
                                    </CardHeader>
                                    <CardContent>
                                        <div className="grid gap-4 sm:gap-6">
                                            {experienceRanges?.map((option, idx) => (
                                                <div
                                                    key={idx}
                                                    className={`border rounded-md p-3 flex items-center justify-between gap-8 hover:bg-muted cursor-pointer ${formData.bussinessYearExprience === option?.value ? "border-blue-700 bg-blue-50" : ""}`}
                                                    onClick={() => handleBussinessYearExprience(option?.value)}
                                                >
                                                    <div className="grid gap-1">
                                                        <Label className='text-blue-800'>{option?.label}</Label>
                                                    </div>
                                                    <div className={`w-5 h-5 rounded-full border ${formData.bussinessYearExprience === option?.value ? 'bg-blue-700 border-blue-700' : 'border-black'}`} />
                                                </div>
                                            ))}
                                            {errors.bussinessYearExprience && <p className="text-red-500 text-sm">{errors.bussinessYearExprience}</p>}
                                        </div>
                                    </CardContent>
                                    {
                                    ...
                                    <CardContent>
                                        <div className="grid gap-4 sm:gap-6">
                                            {professionOptions.map((option, idx) => (
                                                <div
                                                    key={idx}
                                                    className={`border rounded-md p-3 flex items-center justify-between gap-8 hover:bg-muted cursor-pointer ${formData.profession === option.value ? "border-blue-700 bg-blue-50" : ""
                                                        }`}
                                                    onClick={() => setFormData({ ...formData, profession: option.value })}
                                                >
                                                    <div className="grid gap-1">
                                                        <Label className='text-blue-800'>{option.label}</Label>
                                                    </div>
                                                    <div
                                                        className={`w-5 h-5 rounded-full border ${formData.profession === option.value
